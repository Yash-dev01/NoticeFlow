...
export default function NoticeCard({
  ...
  return (
    <div
      className="bg-white rounded-2xl shadow-sm border-2 border-slate-200 overflow-hidden hover:shadow-2xl hover:border-blue-300 transition-all duration-300 group transform hover:-translate-y-1"
    ...
    >
        ...
        <div className="relative h-52 overflow-hidden">
          ...
          {priority === "Urgent" && (
            <div className="absolute top-4 right-4">
              <span className={`inline-flex items-center px-3.5 py-2 rounded-full text-sm font-bold bg-red-500 text-white shadow-2xl backdrop-blur-sm animate-pulse`}>
      ...
      <div className="p-6">
        ...
        <div className="flex items-start justify-between mb-4 gap-3">
          ...
          {priority === "Urgent" && !imageUrl && (
            <span className={`inline-flex items-center px-3 py-1.5 rounded-full text-sm font-bold ${priorityStyles.badge} flex-shrink-0 animate-pulse`}>
              <span className="w-2 h-2 bg-white rounded-full mr-2" />
              Urgent
            </span>
          )}
        ...
        <p className="text-slate-600 text-base leading-relaxed line-clamp-3 mb-5">
          {body}
        </p>
        ...
        <div className="flex items-center justify-end gap-3 pt-5 border-t-2 border-slate-100">
          <Link
            href={`/notices/${id}`}
            className="inline-flex items-center px-4 py-2.5 text-sm font-semibold text-slate-700 bg-gradient-to-r from-amber-400 to-yellow-500 hover:from-amber-500 hover:to-yellow-600 text-white shadow-md shadow-amber-500/20 hover:shadow-lg hover:shadow-amber-500/30 rounded-xl transition-all group/btn"
          >
